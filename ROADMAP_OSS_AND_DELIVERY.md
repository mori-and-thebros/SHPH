...
- Native Windows route/DNS execution still requires operator validation on a
  privileged Windows host; the signed Wintun loader/session/packet backend is
  wired into `TunDevice`, and Linux cross-target check, Clippy, build, and
  Windows test-artifact compilation now pass. Native Windows TUN remains
  host-gated until it is validated on a real Windows host.
...
**Current status (2026-08-05):** local implementation, hardening,
...
**Current status (2026-08-05):** preparation is in progress, but Phase E is
not complete. The benchmark/evidence bundle is published for
`0.5.0-dev.0`; the release snapshot, final claims freeze, and release tag
remain gated on the outstanding native TUN evidence.
...
- Paired WSL2/Linux and native Windows local benchmark capture is complete for
  workspace version `0.5.0-dev.0`; the full score tables, commands, raw-capture
...
- Real native Linux TUN saturation, two-machine RTT/jitter, injected QUIC-shim
  impairment runs, Windows process CPU/RSS instrumentation, Criterion reports,
  and reviewed native-TUN evidence publication: remaining because they require
  operator hosts, privileges, or external traffic tools.
