...
Workspace version `0.5.0-dev.0`. SHPH is **functional for controlled lab
...
- Windows includes a wired Wintun backend with signed application-local
  loading, elevation checks, bounded rings, packet validation, shared-session
  cloning, bounded event waits, and RAII teardown. `SHPH_TUN_NATIVE=1` remains
  host-gated until a real elevated Windows host verifies the signed runtime,
  adapter lifecycle, and packet path.
- Lab-only controls:
...
Platform-specific validation reports and reproducible commands live under
`docs/evidence/` and `docs/`. Do not treat local benchmark paths or host details
as portable deployment configuration.
