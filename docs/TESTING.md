...
the current CI workflow and the reproducible commands in this document.

## Historical platform-targeted test runs (2026-07-15)

These historical results came from separate Linux and Windows checkouts.
...
- Native Windows validation completed on August 5, 2026:
  `cargo fmt --all -- --check`, `cargo check --workspace --locked`,
  `cargo clippy --workspace --all-targets --locked -- -D warnings`,
  `cargo test --workspace --locked`, and
  `cargo build --workspace --release --locked` all passed.
- The native Windows workspace test run reported 176 passed and 0 failed.
- The paired local benchmark is documented in
  `docs/BENCHMARK_RESULTS_2026-08-05.md` and can be reproduced with
...
- Native Wintun adapter, packet-I/O, route/DNS, and two-machine evidence remain
  separate host-gated release work.
...
- Linux native TUN lifecycle checks:
  ...
  - The Windows backend validates signed application-local Wintun loading,
