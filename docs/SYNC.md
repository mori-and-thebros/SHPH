...
SHPH keeps two trees in sync:

- **Linux working copy** (canonical): `/home/mori/SHPH_working_copy`
- **Windows mirror**: `D:\FUNDING NEEDED\snap-shroud-rs` (mounted at `/mnt/d` in WSL)

The Windows `D:\` drive is reachable from this Linux environment via WSL's
drvfs mount at `/mnt/d`, so files can be mirrored directly.
...
- `Cargo.lock` — regenerated per platform; do not mirror
- `.git/` — per-tree version control (if added later)
...
- drvfs permission bits differ between the two trees, so a `--dry-run` may list
  ...
  content parity.
- Source and documentation files are mirrored; never commit or mirror `target/`.
