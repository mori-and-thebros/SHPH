...
use windows_sys::Win32::System::LibraryLoader::{
    GetProcAddress, LoadLibraryExW, LOAD_LIBRARY_REQUIRE_SIGNED_TARGET,
...
impl WintunApi {
    ...
    pub fn load(path: &Path) -> Result<Self> {
        ...
        // application and system directories and require a signed target.
        let module = unsafe { LoadLibraryExW(wide_path.as_ptr(), null_mut(), flags) };
        if module.is_null() {
            return Err(ShphError::Unsupported(format!(
                "unable to load signed Wintun runtime '{}': Win32 error {}",
